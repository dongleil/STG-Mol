...
def build_en():
    ...
    add_para(doc, 'Molecular representation learning has evolved through three paradigms: **1D sequence representations** (Mol2Vec [12], ChemBERTa [13]), **2D graph representations** (GCN [14], GAT [15], D-MPNN [16], AttentiveFP [17]), and **3D geometric representations** (SchNet [18], DimeNet [19], SphereNet [20]). Each captures distinct physicochemical determinants of activity yet exhibits blind spots, motivating multi-modal fusion.')
    add_para(doc, 'Existing fusion approaches fall into two categories. **Static fusion** (concatenation, weighted sum, gated fusion) is simple but non-adaptive. **Contrastive-pretraining fusion** (MolCLR [21], Uni-Mol [22], GEM [23], KPGT [24]) achieves strong performance but requires **millions of GPU-hours** for pretraining and is prone to overfitting when fine-tuned on small target-specific datasets like NLRP3.')
    ...
    add_para(doc, 'AI has become central to drug discovery: in target identification (PPI GNNs [25], AlphaFold [26]), hit discovery (deep-learning virtual screening [27], generative models [28], reinforcement learning [29]), and preclinical evaluation (deep ADMET models [30], GNN property prediction [31]).')

    add_h2(doc, '2.2  Molecular Representation Learning')
    add_para(doc, '**1D representations**: Morgan/ECFP [32], Mol2Vec [12], ChemBERTa [13], MolFormer [33]. **2D representations**: GCN [14], GAT [15], MPNN [34], D-MPNN [16], AttentiveFP [17]. **3D representations**: SchNet [18], DimeNet [19], SphereNet [20], EGNN [35], PaiNN [36]. The paradigms are complementary but each has blind spots.')

    add_h2(doc, '2.3  Multi-Modal Molecular Fusion')
    add_para(doc, '**Static fusion**: concatenation, gated fusion [37], bilinear pooling [38], MMFuse [39]. **Contrastive pretraining fusion**: MolCLR [21], Uni-Mol [22], GEM [23], KPGT [24].')
    ...
    add_para(doc, '**Docking-based conventional pipelines** (AutoDock Vina [40], Glide [41], GOLD [42], DOCK 3.7 [43]) are computationally expensive per molecule. **Machine-learning-accelerated screening** (DeepChem [44], ChemProp [29], Deep Docking [46], MolPAL [47]) offers higher throughput. **Cascaded architectures** (V-SYNTHES [48], Graff [33]) combine multi-stage strategies with deep learning.')

    add_h2(doc, '2.6  Computational Discovery of NLRP3 Inhibitors')
    add_para(doc, 'Known NLRP3 inhibitors span three classes: **MCC950-type sulfonylureas** [9], **CY-09-type thioureas** [50], and **natural-product-type** compounds (oridonin [51], tranilast [52]). Computational studies are largely limited to docking or classical QSAR, lacking end-to-end AI + complete multi-level validation + joint activity-ADMET prediction.')
    ...
    add_para(doc, 'SMILES strings are mapped by a pretrained Mol2Vec [12] model to fragment embeddings, mean-pooled to a 300-dimensional vector, then projected to a fusion dimension d = 112.')

    add_h3(doc, '3.3.2  2D Topological Graph Encoder (D-MPNN)')
    add_para(doc, 'We adopt D-MPNN [16], which passes messages along directed edges. **Atom features are extended to 47 dimensions**, including Gasteiger partial charges, atomic polarizability, and pharmacophore tags (H-bond donor/acceptor, hydrophobe, aromatic, ionisable). T = 3 message-passing steps, hidden dimension 112.')

    add_h3(doc, '3.3.3  3D Geometric Conformer Encoder (SphereNet)')
    add_para(doc, 'SphereNet [20] encodes 3D conformers. Conformers are generated by ETKDGv3 with MMFF94s optimisation; the lowest-energy conformer is retained. T = 3 interaction blocks, K = 6 radial basis functions, spherical basis = 7, cutoff 8 Å.')
    ...
    add_para(doc, 'To address the NLRP3 positive/negative imbalance (~1:3), we adopt **Focal Loss** [63] with **class balancing weights**:')
    ...
    add_para(doc, '**Benefit 1 (Regularisation).** Sharing low-level representations across tasks provides strong regularisation; 5-seed standard deviation dropped from 0.0134 to 0.0077. **Benefit 2 (Performance gain).** Primary Test ROC-AUC improved from 0.9548 to **0.9591**, MCC from 0.7770 to **0.8502** (+0.0732), F1 from 0.8403 to **0.8929**. **Benefit 3 (Clinical value).** A single training produces activity + five drug-likeness predictions, providing a concrete implementation of the **"activity and safety in parallel"** paradigm and directly echoing the MCC950 phase-II termination due to hepatotoxicity.')
    ...
    add_para(doc, '**(ii) Early recognition** (critical for virtual screening): BEDROC@α [Truchon & Bayly, 2007] with α ∈ {20, 80, 160} (α = 20 is our primary and follows the original recommendation; α = 80, 160 are reported as supplementary robustness across the very-early-recognition regime). BEDROC ∈ [0, 1] with theoretical upper bound 1.0.')
    ...
    add_para(doc, 'Category 2: **Single-modality deep learning methods** — ChemBERTa [13], AttentiveFP [17], D-MPNN [16], SchNet [18].')
    add_para(doc, 'Category 3: **Multi-modal / large-scale pretraining methods** — MolCLR [21], Uni-Mol [22], GEM [23], MMFuse [39].')
    ...
    add_para(doc, 'Following the MoleculeNet convention [Wu et al., 2018] and recent virtual-screening reproducibility literature [Tossou et al., 2023], we report performance under **two complementary evaluation protocols on the same curated 2,521-molecule NLRP3 dataset**: (i) **Bemis–Murcko scaffold split** (V3-scaffold, primary protocol) — a stringent test of generalisation to novel chemical series, adopted as the headline benchmark; and (ii) **random split** (V3-random, reference protocol) — reported alongside to characterise the upper bound of in-distribution performance and to enable comparison with prior work that used random splitting. Both protocols share identical data curation (removal of five external hold-out inhibitors and their Tanimoto ≥ 0.7 neighbours) and identical model / training hyperparameters. **Model selection (5-seed ensemble weights, decision threshold) was performed on the validation set of each split independently; the test set was not used for any model or data selection decision.** Table 5.1 summarises test-set performance across categories of baselines.')
    add_caption(doc, 'Table 5.1  STG-Mol vs. baselines on the NLRP3 test set — primary protocol: scaffold split (5-seed mean ± std)')
    ...
    add_para(doc, '**(i) Substantial improvement in classification decision quality**: although Ensemble ROC-AUC is essentially identical (0.9590 vs. 0.9591)—indicating equivalent **ranking ability**—the **decision-threshold-based metrics** (F1, MCC, Recall) all improve significantly: F1 +0.0202, MCC +0.0279, Recall +0.0370. This suggests that the regularisation provided by the ADMET auxiliary tasks pushes the decision boundary closer to what virtual screening actually needs.')
    ...
    add_caption(doc, 'Table 5.5  Hierarchical fusion component leave-one-out ablation (V3-random 5-seed mean ± std)')
    ...
    add_caption(doc, 'Table 5.4  External evaluation: predicted probabilities and Tanimoto analysis for five published NLRP3 inhibitors')
    ...
    add_para(doc, '**Deployment recommendation — AD-gated screening.** We therefore propose deploying STG-Mol as an **in-AD screener** paired with a complementary OOD channel. Concretely, in the large-scale screening protocol of Section 5.5 we (i) compute the nearest-neighbour Tanimoto to training for each library compound, (ii) route in-AD compounds (Tanimoto ≥ 0.4) through STG-Mol\'s classifier for ranking, and (iii) subject deep-OOD compounds (Tanimoto < 0.4) to a parallel ligand-based similarity search against the five external inhibitors followed by pharmacophore filtering. This AD-gated protocol converts what a naïve reader might view as a "3/5 miss" into a **transparent, structured deployment envelope**: within the AD, STG-Mol\'s high-confidence predictions can be trusted; outside the AD, the framework declines to make confident calls and defers to orthogonal evidence. We report both the recall and the deployment strategy openly rather than concealing the OOD miss.')
    ...
    add_caption(doc, 'Table 5.7. Docking summary of the eight prioritised candidates.')
    ...
    add_para(doc, 'To assess whether the candidate ranking is robust to model refinements, we re-evaluated the 8 candidates with the V3-random 5-seed ensemble described in Section 5.1. **Table 5.11** presents activity probabilities, 5-seed ranges, Tanimoto distances to training, and AD categories:')
    add_caption(doc, 'Table 5.11. V3-random 5-seed ensemble consistency validation on the 8 candidates.')
    ...
    add_para(doc, '**Analysis.** (i) **Ranking consistency across evaluation protocols**: 7/8 (87.5%) candidates are independently confirmed as predicted active by the V3-random 5-seed ensemble, and the three strongest Vina binders (Compounds 1, 2, 8; ΔG < -9.4 kcal/mol) **all pass the threshold** — evidencing that the candidate ranking is stable under alternative splitting protocols. (ii) **Structural novelty**: mean Tanimoto = 0.251, all < 0.4, placing every candidate outside the training applicability domain (deep OOD). These candidates were retrieved via the AD-gated deployment strategy of Section 5.4 — i.e., predictions were combined with orthogonal ligand-based similarity and pharmacophore evidence, and their computational validation in Sections 5.6.3–5.6.5 is the primary support, not the raw activity probability alone. (iii) **Compound 5 abstention** (prob = 0.407, 5-seed range 0.074–0.816) exemplifies the AD-aware confidence behaviour described in Section 5.4: the model expresses substantial uncertainty on this compound and does not commit to a confident positive call. We flag this as a lower-priority target for wet-lab prioritisation until orthogonal evidence is obtained. **Caveat.** Because all eight candidates are deep OOD, the V3-random probabilities alone are not sufficient evidence of activity; the consistency shown here should be read as an internal robustness check on the ranking, not as a validation of biological activity. Prospective wet-lab confirmation remains essential and is planned as future work (Section 6.5).')
