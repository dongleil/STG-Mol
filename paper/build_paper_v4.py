...
def build_zh():
    ...
    add_para(doc, '**(ii) 早期识别能力**（虚拟筛选核心）：BEDROC@α [Truchon & Bayly, 2007]，α ∈ {20, 80, 160}（α = 20 为主指标，遵循原始推荐；α = 80、160 作为极早期识别区间的稳健性补充）。BEDROC ∈ [0, 1]，理论上限 1.0。')
    ...
    add_para(doc, '遵循 MoleculeNet 惯例 [Wu et al., 2018] 及近期虚拟筛选可复现性文献 [Tossou et al., 2023]，我们**在同一策划的 2521 分子 NLRP3 数据集上并列报告两种互补协议**：(i) **Bemis–Murcko scaffold split（V3-scaffold，主协议）**——对新颖化学骨架泛化能力的严格测试，作为 headline 评估基准；(ii) **Random split（V3-random，参考协议）**——量化在分布内性能上限，同时便于与采用随机划分的既有工作对齐。两种协议共用相同的数据策划流程（移除 5 个外部抑制剂及其 Tanimoto ≥ 0.7 邻居）与相同的模型/训练超参数。**模型选择（5-seed 集成权重、决策阈值）分别在两种协议的验证集上独立完成；测试集未参与任何模型或数据决策**。表 5.1 给出各类别基线与 STG-Mol 的对比。')
    add_caption(doc, '表 5.1  STG-Mol 与基线方法在 NLRP3 测试集上的对比——主协议：scaffold split（5-seed mean ± std）')
    ...
    add_caption(doc, '表 5.5  分层融合模块 leave-one-out 消融（V3-random 5-seed mean ± std）')
    ...
    add_caption(doc, '表 5.4  外部评估：5 个已发表 NLRP3 抑制剂的预测结果与 Tanimoto 分析')
    ...
    add_caption(doc, '表 5.7  8 个候选化合物的分子对接与结合位点信息')
    ...
    add_para(doc, '为验证候选化合物 ranking 对模型细节的鲁棒性，我们使用第 5.1 节 V3-random 5-seed 集成模型对 8 个候选进行独立预测。**Table 5.11** 展示 activity 概率、5-seed 一致性、Tanimoto 相似度与 AD 类别：')
    add_caption(doc, '表 5.11  V3-random 5-seed 集成模型对 8 candidates 的独立验证')
    ...
    add_para(doc, '**分析要点**：(i) **跨评估协议的 ranking 一致性**——7/8 (87.5%) 化合物被 V3-random 5-seed 集成模型独立确认为 predicted active，且三个最强 Vina binders (Compound 1/2/8, ΔG < -9.4 kcal/mol) **全部通过阈值**，证明候选 ranking 在不同划分协议下保持稳定。(ii) **结构新颖性**：所有 8 个候选平均 Tanimoto = 0.251，均 < 0.4，位于训练集应用域之外（deep OOD）。这些候选来自第 5.4 节所述的 AD-gated 部署策略——即预测与正交的配体相似度与药效团证据结合，其 5.6.3–5.6.5 节的计算验证是主要支撑，而非仅凭活性概率。(iii) **Compound 5 弃权** (prob = 0.407, 5-seed range 0.074–0.816) 体现了 5.4 节所述的 AD-aware 置信度行为：模型对该化合物表达显著不确定性，未给出高置信正判定。我们在获得正交证据前将其列为湿实验优先级较低的目标。**警示**：由于 8 个候选全部处于 deep OOD，V3-random 概率本身**不构成活性的充分证据**；此处一致性应视为对 ranking 的内部稳健性检查，而非活性的验证。前瞻性湿实验确认仍必需，已列入未来工作（6.5 节）。')
